...
export function getPreviewImageUrl(id: string): string {
  return `/api/documents/${id}/preview-image`
}
...
export async function ocrRegion(
  ...
  const res = await apiClient.post(`/documents/${id}/ocr-region`, {
...
): Promise<{ success: boolean; extracted_fields: Record<string, unknown> }> {
  const res = await apiClient.put(`/documents/${id}/fields`, { fields })
  return res.data
}

export async function getDocumentCorrections(
  id: string
): Promise<{ sender: string | null; corrections: { field_name: string; human_value: string; count: number }[] }> {
  const res = await apiClient.get(`/documents/${id}/corrections`)
  return res.data
}
